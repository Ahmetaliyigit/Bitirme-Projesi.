...
{
    public partial class TisortAl : Form
    {
        ...
        private void btn_Ekle_Click(object sender, EventArgs e)
        {
            ...
            if (Donus == true)
            {
                if (user != null)
                {
                    var urun = Db.Tshirts.Where(k => k.Id == Convert.ToInt32(txt_Id.Text)).FirstOrDefault();

                    var Sifirmi = Db.Jackets.Where(k => k.ConditionId == urun.ConditionId).FirstOrDefault();
